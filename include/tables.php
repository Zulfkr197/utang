<?php
	...
	$zfhtnggetblanhari	= (empty($_POST['hari'])) ? "-".date('m') : $zfhtnggetblan;
	$zfhtnggettahun		= $_POST['tahun'];
	if(!empty($_POST['hari']) && empty($_POST['bulan'])){ // jika hari = tidak kosong, jika bulan = kosong
		$zfhtngfiltrblan 	= (empty($_POST['hari'])) ? "AND tanggal LIKE '$zfhtnggettahun$zfhtnggetblanhari%'" : "AND tanggal='$zfhtnggettahun-".date('m')."$zfhtnggethari'";
	}else{ // jika hari = kosong, jika bulan = tidak kosong
	...
	$zfhtngquery		= mysqli_query($sqlconn,"SELECT `tanggal`, `pelanggan`, `jumlah`, `bayar`, `ket`, `id` FROM `utang` WHERE `status`='$zfhtngvalstat' $zfhtngfiltrplgn $zfhtngfiltrblan ORDER BY tanggal DESC");
	...
	if(!mysqli_num_rows($zfhtngquery)){
		...
		while($row = mysqli_fetch_row($zfhtngquery)){
			...
			$sisa		= (empty($row[2] - $row[3])) ? "<div class='bg-green-500 text-white px-4 py-2 w-full rounded-md text-base font-medium'>LUNAS</div>" : number_format(($row[2] - $row[3]),0,",",",");

			if(($row[2] - $row[3]) == 0 || ($row[2] - $row[3]) < 0){
				$tipytxt	= 'data-tippy-content="Lunas" data-tippy-placement="left"';
				$clortxt	= 'gray-700';
			}else{
				$clortxtcond	= (empty($_POST['bulan'])) ? 'red-700 font-medium' : 'gray-700';
				$tipytxt	= ((date('j') - $dates[2]) > 14) ? 'data-tippy-content="Lebih dari '.(date('j') - $dates[2]).' Hari" data-tippy-placement="left"' : '';
				$clortxt	= ((date('j') - $dates[2]) > 14) ? $clortxtcond : 'gray-700';
			}
			...
			echo "
			<div class='post text-lg lg:text-lg bg-white border border-gray-200 grid grid-cols-11 lg:gap-4 text-gray-700 pt-2 my-6 lg:px-0 lg:py-4 lg:my-0 items-center rounded-md lg:rounded-none' data-id='$row[5]' $tipytxt>
				<div class='col-span-12 lg:col-span-3 lg:pl-4 lg:py-4 text-gray-700'>
					<div class='lg:flex lg:gap-x-6 capitalize'>
						<div class='flex w-full lg:w-1/4 lg:text-center py-4 px-8 border-b-2 border-gray-200 lg:p-0 lg:border-0'>
							...
							<div class='w-3/4 pl-4 lg:pl-0 lg:w-auto text-$clortxt'>$tanggal</div>
						</div>
						<div class='flex w-full lg:w-3/4 overflow-ellipsis overflow-hidden py-4 px-8 border-b-2 border-gray-200 lg:p-0 lg:border-0'>
							...
							<div class='w-3/4 pl-4 lg:pl-0 lg:w-auto text-$clortxt'>$pelanggan</div>
						</div>
					</div>
				</div>
				<div class='col-span-12 lg:col-span-5 lg:px-2 lg:py-2'>
					<div class='lg:flex lg:gap-x-6'>
						...
						<div class='flex w-full lg:w-1/3 py-4 px-8 border-b-2 border-gray-200 lg:p-0 lg:border-0 items-center'>
							...
							<div class='w-3/4 pl-4 lg:pl-0 lg:w-auto editsisa text-$clortxt'>$sisa</div>
						</div>
					</div>
				</div>
					...
					<div class='lg:flex lg:gap-x-4'>
						...
						<div class='flex w-full lg:w-2/6 py-4 px-8 border-b-2 border-gray-200 rounded-b-md lg:p-0 lg:border-0'>
							...
							<div class='w-3/4 pl-4 lg:pl-0 flex gap-x-3 w-auto lg:w-full h-full pr-0 lg:pr-2'>
								$lnsbtn
								<button class='trashitem bg-red-500 hover:bg-red-600 hover:text-gray-100 text-white text-sm py-4 lg:py-2 $lnstrsh rounded text-center' data-tippy-content='Hapus'><svg class='w-5 h-5 mx-auto' xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16' /></svg></button>
							</div>
						</div>
					</div>
				</div>
